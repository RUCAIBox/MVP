...
class AbstractDataLoader(object):
    ...
    def __next__(self):
        if (self.drop_last
            and self.std_pr + self.batch_size >= self.pr_end) or (not self.drop_last and self.pr >= self.pr_end) or (self.batch_test is not None and self.std_pr // self.batch_size >= self.batch_test):
            self.pr = 0
            self.std_pr = 0
            raise StopIteration()
    ...
    def _shuffle(self):
        ...
        for key, value in self.dataset.__dict__.items():
            if key.startswith(('source', 'target')) and isinstance(value,
                                                                   list) and isinstance(value[0], (list, str, int, tuple)):
                keys.append(key)
                values.append(value)
        values = list(zip(*values))
        if self.batch_test:
            values.sort(key=lambda x:len(x[0])+len(x[1]), reverse=True)
        else:
    ...
    def get_reference(self):
        ...
        if self.config['tokenize_strategy'] == 'none':
            if isinstance(target_text[0], list):
                return [[t.split(' ') for t in tar] for tar in target_text]
