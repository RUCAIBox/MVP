...
from mvp.utils import get_model, Enum, general_arguments, training_arguments, \
...
class Config(object):
    ...
    def _load_cmd_line(self):
        ...
        if len(unrecognized_args) > 0:
            logger = getLogger()
            logger.warning('command line args [{}] will not be used'.format(' '.join(unrecognized_args)))
    ...
    def __str__(self):
        args_info = '\n'
        logger = getLogger()
        logger.info(self.final_config_dict)
