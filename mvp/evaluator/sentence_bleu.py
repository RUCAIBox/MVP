...
def sentence_bleu(
    ...
    >>> weights = {'bleu-5-avg': (1./5., 1./5., 1./5., 1./5., 1./5.)}
    ...
    :type weights: dict<str, tuple(float)>
...
def corpus_bleu(
    ...
    # order of n-grams < 4 and weight is set at default.
    if auto_reweigh:
        if hyp_lengths < 4 and weight == (0.25, 0.25, 0.25, 0.25):
            weight = (1 / hyp_lengths,) * hyp_lengths

    # Collects the various precision values for the different ngram orders.
    p_n = [Fraction(p_numerators[i], p_denominators[i], _normalize=False) for i, _ in enumerate(weight, start=1)]
